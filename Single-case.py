...
class Stenosis_2(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        y_case = torch.where(
            ...
            torch.where(
                mask2,
                y_ref * (1 - self.fc * (1 + torch.cos(x_ref_transformed * torch.pi))),
                y_ref
            )
        )
...
class GE(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        radius = torch.where(
            ...
            torch.where(
                mask2,
                y_case / (1 - self.fc * (1 + torch.cos(x_ref_transformed * torch.pi))) /self.r,
                y_case / self.r
            )
        )
...
def run(cfg: ModulusConfig) -> None:
    ...
    Stenosis_coordTransform=CustomModuleArch(
        ...
        module=Stenosis_2(channel_radius_nd,channel_length_nd[1]-channel_length_nd[0],fc)
        )
    ...
    ge_net=CustomModuleArch(
        ...
        module=GE(channel_radius_nd,channel_length_nd[1]-channel_length_nd[0],fc)
        )
    ...
    nodes=nodes, invar=openfoam_invar_numpy, output_names=["u", "v", "p",'warpx','warpy',"cline","radius"]
        ...
        domain.add_inferencer(openfoam_inferencer, "PinnSingle")   
    else:
